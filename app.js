...
async function BookPurchasing(bookName, price, stock, bookAmount, discount, tax) {
...
async function CalculatingTerm(totalPrice, creditDuration, additionalPrice, payDate) {
  // let arrTotalPrice=[];
  const currentDate = new Date();  // date currently

  const restPrice = totalPrice % creditDuration;
  let listTermAmount = new Set();
  let listTerm = new Map();

  // arrTotalPrice.push(totalPrice);
  // let invoiceEachMonth = arrTotalPrice.map((price, index) => index === arrTotalPrice.length-1 ? Math.floor(price/creditDuration) : price); 
  let invoiceEachMonth = Math.floor(totalPrice/creditDuration)
  ...
  for (let i = 1; i <= creditDuration; i++) {
      ...
      if (additionalPrice >= 0 && i == creditDuration){
        invoiceEachMonth = invoiceEachMonth + restPrice + additionalPrice;
        listTermAmount.add(invoiceEachMonth);
        listTerm.set(dueDate.toDateString(), {dueDate: dueDate.toDateString(), invoiceEachMonth: invoiceEachMonth});
      } else {
        listTermAmount.add(invoiceEachMonth);
        listTerm.set(dueDate.toDateString(), {dueDate: dueDate.toDateString(), invoiceEachMonth: (invoiceEachMonth)});
      }
    ...
    const listTermAmountPrice = Array.from(listTermAmount);

    // convert map object to an object
    const listTerms = {};
    listTerm.forEach((value, key) => {
      listTerms[key] = value;
    });

    // Get certain data using Map
    let termToPay = listTerm.get(payDate);
    if (termToPay === undefined){
      termToPay = {};
    }
    // console.log(listTerm)

    // const termToPay = listTerms[payDate];
    // console.log(termToPay)
...
app.get('/book-purchase', basicAuthMiddleware, async (req, res) => {
  ...
  const { bookName, price, stock, bookAmount, discount, tax, creditDuration, additionalPrice, payDate} = req.body;
  ...
  try {
    ...
    const calculate = await CalculatingTerm(totalPrice, creditDuration, additionalPrice, payDate);
